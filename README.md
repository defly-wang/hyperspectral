...
### 数据查看
- **多格式支持**: 支持读取 `.isf`、`.xlsx/.xls` 格式的高光谱数据，以及 USGS 光谱库 `.txt` 文件
- **USGS光谱库**: 内置USGS光谱库浏览功能，支持按类别/子类别筛选，中英文双语显示
- **光谱曲线显示**: 可视化波长与反射/透射率关系曲线
- **多文件查看**: 支持单选和多选文件，多选时在同一图表中显示多条曲线
- **图例拖动**: 光谱图例可用鼠标拖动到任意位置
- **曲线选择**: 鼠标点击曲线可选中/取消选中，选中曲线高亮加粗
- **伪彩色图像**: 将光谱数据转换为伪彩色图像展示，多选时水平排列显示
  - 子图间距设为0，隐藏边框实现"融合"效果
  - 点击图片可在顶部显示文件名
- **文件排序**: 文件列表按文件名排序
...
- 自动从目录加载训练数据（按子目录分类）
- 支持直接加载 USGS 光谱库目录作为训练数据
- 训练结果显示准确率、分类报告
- 模型保存与加载功能

### 识别
- 加载训练好的模型进行预测
- 支持识别本地文件(.isf, .xlsx, .xls)和 USGS 光谱文件(.txt)
- 支持批量识别多个文件
- 显示识别结果及置信度
- 表格形式展示文件列表，包含文件名、识别结果、置信度
